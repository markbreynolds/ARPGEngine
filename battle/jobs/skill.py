...
class WaveSlash(ProjectileSkill):
	...
	def getChanges(self):
		return [str((self.level+1)*self.dmg)+" Damage (+4)",str(self.level+1)+" MP Cost (+1)"]

	## See Skill.getInfo().
	def getInfo(self):
		return "Deals "+str(self.level*self.dmg)+" base damage"
	...
	def levelUp(self):
		self.level += 1
		self.projectile.setDamage(self.dmg*self.level)
